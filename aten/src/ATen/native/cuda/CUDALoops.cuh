...
__global__ void vectorized_templated_elementwise_kernel(int N, func_t f, array_t data,
  ...
  if (remaining < vectorized_templated_config::block_work_size()) { // if this block handles the reminder,
                                       // just do a naive unrolled loop
    auto policy = memory::policies::unroll_parametrized<
    ...
    templated_unrolled_elementwise_kernel_helper<vectorized_templated_config::thread_work_size()>(f, policy);
  } else { // if this block has a full `block_work_size` data to handle, use
           // vectorized memory access
...
const c10::ScalarType rt_binary_specializations[4][2] =
